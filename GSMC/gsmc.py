...
@torch.jit.script
def to_homogeneous_torch(pts, coord_axis: int = -1, device: str = 'cuda'):
    if coord_axis == 0:
...
# @my_timeit
@torch.jit.script
def compute_visibility_mask_torch(p3D, ctr_array, thr_low, thr_up, mid_camera_directions, thr_cos_angle):
    # , device="cuda",
...
# @my_timeit
def compute_projections(X_coords, ctr_array, phi, g_direction, x_dir, p3D, intrinsics, dist_coefs, n_angles=360,
                        device="cuda"):
    ...
    p2p1 = torch.from_numpy(dist_coefs[:-3:-1].copy()).to(device=device, dtype=torch.float32)
    dist_coefs = torch.from_numpy(dist_coefs).to(device=device, dtype=torch.float32)
    rt_array = torch.from_numpy(rt_array).to(device=device, dtype=torch.float32)
    p3D_h = torch.from_numpy(p3D_h).to(device=device, dtype=torch.float32)
...
# @my_timeit
@torch.jit.script
def compute_projections_torch(rt_array, p3D, intrinsics, dist_coefs, device:str="cuda"):
    '''Version vectorising the computation for all temptative cameras associated to one match
    :param ctr_array:
    '''
    ...
    #p2p1 = torch.tensor([dist_coefs[-1], dist_coefs[-2]], device=device)
    p2p1 = dist_coefs[[-1,  -2]].to(device)
...
def get_match_specific_params(match_2d_hcoords, match_3d_coords, g_direction, z0, camera_internals, n_angles):
    ...
    alpha = np.arccos(
        np.array(g_direction).dot(x_direction)) - np.pi / 2  # TODO: adding normalization. Verify it is due
...
def compute_GSMC_score_compiled(match_2d_coords, match_3d_id, z0, g_direction, camera_internals, dist_coefs, query_mask,
                                points_3d_coords, point_cloud_info, n_angles=360):
    ...
    x_direction, alpha, R, phi, ctr_array = get_match_specific_params(match_2d_hcoords, match_3d_coords, g_direction,
                                                                      z0, camera_internals, n_angles)
...
def compute_gsmc_score_torch(match_pts_2d, match_pts_3d, point_cloud_info, z0, g_direction, camera_internals,
    ...
    # load semantic data
    pc_labels = torch.tensor([v.semantic_label for _, v in point_cloud_info.items()],
