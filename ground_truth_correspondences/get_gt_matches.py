...
def get_gt_matches(query_name, slicepath, slice, qkp=None, full_points3D=None, visibility_check=True,
                   return_prjpts=False, proj_pts_dict=None):

    '''
    Return n2Dkeypts x n3Dpts matrix with entries representing the negative exponential of the distance of the 2D keypoint and the 3D point projection.
    To be thresholded to define ground truth matches.
    '''
    ...
    if visibility_check and proj_pts_dict is not None:
        ...
        partial_mask = np.logical_and(np.greater(ssd, (0.9 * proj_pts_dict['dlow']) ** 2),
                                      np.greater((1.1 * proj_pts_dict['dup']) ** 2, ssd))
        ...
        p2D = cv.projectPoints(proj_pts_dict['coord'][pid_mask], cv.Rodrigues(R_gt_query)[0], t_gt_query, intrinsics,
                               np.array(dist_coefs))[0].reshape((np.sum(pid_mask), 2))
