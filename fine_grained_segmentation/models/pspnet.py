...
from fine_grained_segmentation.models.resnet import _ConvBatchNormReLU, _ResBlock
from fine_grained_segmentation.models.resnet_orig import ResNetForPsp, Bottleneck
...
class _PyramidPoolModule(nn.Sequential):
    ...
    def __init__(self, in_channels, pyramids=[6, 3, 2, 1], use_bn=True):
        ...
        ## regular list of modules, which however are properly recognized as such
...
class PSPNet(nn.Module):
    ...
    def __init__(self, n_classes=19, n_blocks=[3, 4, 23, 3], pyramids=[6, 3, 2, 1], input_size=[713, 713], use_bn=True, output_features=False, output_all=False, use_original_base=False):
        ...
        self.output_features = output_features # ? whether to return features?
...
class PSPNetClustering(nn.Module):
    ...
    def __init__(self, n_classes=19, n_blocks=[3, 4, 23, 3], pyramids=[6, 3, 2, 1], input_size=[713, 713], use_bn=True, output_features=False, output_all=False, use_original_base=False):
        ...
        self.input_size = input_size
        self.n_classes = n_classes
        self.output_features = output_features ## Assuming that the output_all is false, otherwise that rules:
        ## only output features (aux/not at training time, only normal at inference) if true, otherwise only output outputs
        self.output_for_cluster = False
        self.output_all = output_all # outputs all features of the model (and the auxiliary model if at training time). But the point is there are both features and outputs
        if use_original_base:
            self.fcn = ResNetForPsp(Bottleneck, n_blocks)
        else:
            self.fcn = _DilatedFCN(n_blocks=n_blocks, use_bn=use_bn)
...
if __name__ == '__main__':
    model = PSPNetClustering(n_classes=19, n_blocks=[3, 4, 6, 3], pyramids=[6, 3, 2, 1], use_original_base=True)
    # model = PSPNet(n_classes=19, n_blocks=[3, 4, 6, 3], pyramids=[6, 3, 2, 1], use_original_base=True)
    summary(model, (3,713, 713))
